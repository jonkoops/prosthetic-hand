...
export default class Hand {
	...
	fingerIsBusy() {
		...
		if (this._fingersAreIdle) {
			...
			// Fired when a movement is about to start.
	...
	_dispatchEvents(timestamp) {
		...
		// Fired when a movement is about to start, just before the
		// mouse/touch/pointer events are fired.
		document.dispatchEvent(new CustomEvent("prostheticHandStart", { target: this }));
		...
		var fast =
			this._timingMode === enums.MINIMAL || this._timingMode === enums.INSTANT || this._timingMode === enums.FASTFRAME;
	...
	_createTouchEvent(type, data) {
		if (capabilities.touchEventConstructor) {
			...
			if (touchEvent && touchEvent.initTouchEvent) {
				if (touchEvent.initTouchEvent.length == 0 && !capabilities.safari) {
					// Chrome
