...
export default class Finger {
	...
	// `Touch`es.
	_asTouch() {
		if (!capabilities.touchConstructor) {
			throw new Error("Browser can't instantiate Touches");
		}
		let touch = new Touch({
			identifier: this._id,
	...
	_asPointerEvent(evType) {
		if (!capabilities.pointerEventConstructor) {
			throw new Error("Browser can't instantiate PointerEvents");
		}
		const ev = new PointerEvent("pointer" + evType, {
			bubbles: true,
	...
	_asMouseEvent(evType) {
		if (!capabilities.mouseEventConstructor) {
			throw new Error("Browser can't instantiate MouseEvents");
		}
		const ev = new MouseEvent("mouse" + evType, {
			bubbles: true,
			button: 0, // Moz doesn't use -1 when no buttons are pressed, WTF?
			buttons: this._state.down ? 1 : 0,
			detail: evType === "down" || evType === "up" ? 1 : 0, // TODO: count consecutive clicks
